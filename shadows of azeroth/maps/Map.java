    ...
    public class Map {
        ...
        public void displayCurrentMap(Player player) {
            ...
            // Рисуем карту
            drawMap(currentMap, size, player.getX(), player.getY(),"🧙‍♂️");
        }
        ...
        private void drawMap(Cell[][] map, int size, int playerX, int playerY,String player_emoji) {
            for (int x = 0; x < size; x++) {
                for (int y = 0; y < size; y++) {
                    if (x == playerX && y == playerY) {
                        System.out.print("🧙‍♂️"); // Символ игрока (можно заменить на любой эмодзи)
                    } else {
        ...
        private void drawEditorMap(Cell[][] map) {
            int size = map.length;
            for (int x = 0; x < size; x++) {
                for (int y = 0; y < size; y++) {
                    System.out.print(" " + map[x][y].getCelltype() + " ");
